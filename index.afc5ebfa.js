...
if(!t.ok)throw Error(`Network response was not OK - HTTP error: ${t.status}`);let o=await t.json();return o}catch(e){console.error("There has been a problem with your fetch operation:",e.message)}}}const l=new n,c=document.querySelector(".categories-list");async function d(e){if("LI"!==e.target.nodeName)return;let t=document.querySelector('.categories-list-item[selected="true"]');t&&t.removeAttribute("selected"),e.target.setAttribute("selected","true");let o=e.target.textContent;try{await l.fetchByCategory(o),console.log("Selected category:",o)}catch(e){console.error(e)}}c.addEventListener("click",d),async function(){try{let e=await l.fetchBooksCategoryList();if(e.length<1)return c.innerHTML="Sorry, we didn't find anything",[];let t=e.map(e=>`
    ...
    `).join("");c.innerHTML=`<li class="categories-list-item" selected="true">All categories</li>${t}`,console.log("Categories loaded successfully.")}catch(e){return c.innerHTML=`Error: ${e.message}`,console.error(e),[]}}();const u=new n,p=document.querySelector(".best-sellers");// Feature to download the best books
...
function(e){let t=`
  ...
  `).join("");g.innerHTML=t}(e)}).catch(e=>console.error("Error fetching data:",e))}(o),k.style.display="none"});const b=document.querySelector(".down-btn"),f=document.querySelector(".up-btn"),_=document.querySelector(".charities__list");b.addEventListener("click",()=>{setTimeout(function(){_.scrollTo(0,1e3)},10),b.classList.add("is-hidden"),f.classList.remove("is-hidden")}),f.addEventListener("click",()=>{setTimeout(function(){_.scrollTo(top)},10),f.classList.add("is-hidden"),b.classList.remove("is-hidden")}),i("erv9X"),i("bLbWr");var w={};w=new URL("iconAmazon@x2.93c925d9.png",import.meta.url).toString();var v={};v=new URL("iconAppleBooks@x2.65551b4a.png",import.meta.url).toString();const L=new n,S="storage-book-data";let T={};const E=document.querySelector(".pop-up-modal"),A=document.querySelector(".backdrop-modal"),$=document.querySelector("#allBooks"),H=document.querySelector(".category-books-list"),B=document.querySelector(".best-sellers"),x=document.querySelector(".close-pop-up-modal-btn"),q=document.querySelector(".add-book-button"),z=document.querySelector(".storage-comment"),N=document.querySelector(".remove-button");function C(){E.classList.remove("is-hidden"),A.classList.remove("is-hidden")}function R(){E.classList.add("is-hidden"),A.classList.add("is-hidden")}async function I(e){try{T={};let t=await L.fetchById(e);if(!t)throw Error("Book not found");return T={book_image:t.book_image,title:t.title,author:t.author,category:t.category,description:t.description,marketAmazon:t.buy_links[0].url,marketAppleBooks:t.buy_links[1].url,list_name:t.list_name,id:t._id}}catch(e){throw console.error("Error",e),e}}async function j(t){$.innerHTML="";try{let o=await I(t);!function(t){let o=`
  ...
  <img src="${/*@__PURE__*/e(v)}" alt="AppleBooks">
    ...
    </div>`;$.innerHTML=o}(o);let r=localStorage.getItem(S),i=JSON.parse(r)||[],s=i.find(e=>e.id===t);s?(q.style.display="none",N.style.display="block"):(q.style.display="block",N.style.display="none")}catch(e){console.error("Error",e),$.innerHTML="<p>Sorry, we couldn't find that book. Please try again later.</p>"}}H?H.addEventListener("click",e=>{if(["UL","DIV","H2"].includes(e.target.nodeName))return;let t=e.target.closest("li").getAttribute("data-id");C(),j(t)}):console.error("Element .category-books-list nie został znaleziony!"),B?B.addEventListener("click",e=>{if(["UL","DIV","H2"].includes(e.target.nodeName))return;let t=e.target.closest("a").getAttribute("data-id");C(),j(t)}):console.error("Element .best-sellers nie został znaleziony!"),x.addEventListener("click",R),q.addEventListener("click",function(){let e=JSON.parse(localStorage.getItem(S))||[],t=T;e.push(t),localStorage.setItem(S,JSON.stringify(e)),q.style.display="none",N.style.display="block",z.textContent="Congratulations! You have added the book to the shopping list. To delete, press the button “Remove from the shopping list”."}),N.addEventListener("click",function(){let e=T.id,t=JSON.parse(localStorage.getItem(S))||[],o=t.findIndex(t=>t.id===e);-1!==o&&(t.splice(o,1),localStorage.setItem(S,JSON.stringify(t))),q.style.display="block",N.style.display="none",z.textContent="";let r=new CustomEvent("removeBookFromList",{detail:{bookId:e}});document.dispatchEvent(r)}),A.addEventListener("click",e=>{e.currentTarget===e.target&&R()}),window.addEventListener("keydown",e=>{"Escape"===e.code&&R()});const M=new n;M.fetchByCategory("Business Books").then(e=>{let t=e.map(e=>{let t={_id:e._id,title:e.title,author:e.author,book_image:e.book_image,description:e.description,category:e.list_name,publisher:e.publisher,price:e.price,marketAmazon:e.amazon_product_url,rank:e.rank};return t});return console.log("Business Books: ",t),t}),M.fetchByCategory("Audio Nonfiction").then(e=>{let t=e.map(e=>{let t={_id:e._id,title:e.title,author:e.author,book_image:e.book_image,description:e.description,category:e.list_name,publisher:e.publisher,price:e.price,marketAmazon:e.amazon_product_url,rank:e.rank};return t});return console.log("Audio Nonfiction: ",t),t}),M.fetchBooksCategoryList().then(e=>{let t=e.map(e=>e.list_name);console.log("Category List: ",t)}),M.fetchById("643282b1e85766588626a0b6").then(e=>{let t={_id:e._id,title:e.title,author:e.author,book_image:e.book_image,description:e.description,category:e.list_name,publisher:e.publisher,price:e.price,marketAmazon:e.amazon_product_url,rank:e.rank};return console.log("book by id: ",t),t}),M.fetchTopBooks().then(e=>{// const list = data.map((element) => element.list_name);
console.log("Top List: ",e)});//# sourceMappingURL=index.afc5ebfa.js.map

//# sourceMappingURL=index.afc5ebfa.js.map
